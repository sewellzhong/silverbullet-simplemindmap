...
export async function previewCurrentPage(): Promise<void> {
  ...
  const content = result.ok
    ? renderDocumentWidget(result.value, config, assets)
    : renderErrorWidget(result.error, config.height, assets)

  await editor.showPanel("modal", 1, content.html, content.script)
}
