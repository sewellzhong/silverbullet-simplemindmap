...
import { viewerScript } from "./viewer-script.ts"
...
function viewerHtml(height: number, assets: RenderAssets): string {
  return `${styleTag(assets.mindMapCss)}${styleTag(assets.viewerCss)}<div class="smm-viewer" style="height: ${height}px"><div id="smm-map" class="smm-map"></div><div class="smm-state" id="smm-loading">Loading mind map...</div><div id="smm-note-tooltip" class="smm-note-tooltip" role="tooltip"></div></div>`
}
